...
MyAreaTest::MyAreaTest(int width, int heigth)
{
  set_content_width(width);
  set_content_height(heigth);

  set_draw_func(sigc::mem_fun(*this, &MyAreaTest::onDraw));
}

MyAreaTest::~MyAreaTest()
{
}

void MyAreaTest::onDraw(const Cairo::RefPtr<Cairo::Context>& context, int width, int height)
{
  ...
  surface->write_to_png("images/image.png");
}